...
public class NetworkService {
	...
	@Produces(MediaType.APPLICATION_JSON)
	// Query parameters are parameters:
	// http://92.222.33.38:8080/app_server/ns/register?pseudo=pqrs&password=abc&first_name=xyz&last_name=cdf&email=hij
	public String register(@QueryParam("pseudo") String pseudo,
			@QueryParam("password") String password,
		...
		if (!FieldVerifier.verifyName(pseudo)) {
			...
			obj.put("returnCode", ErrorCode.MISSING_PASSWORD);
		} else if (!FieldVerifier.verifyName(first_name)) {
			obj.put("returnCode", ErrorCode.MISSING_FIRST_NAME);
		} else if (!FieldVerifier.verifyName(last_name)) {
			obj.put("returnCode", ErrorCode.MISSING_LAST_NAME);
		} else if (!FieldVerifier.verifyEMailAddress(email)) {
			obj.put("returnCode", ErrorCode.MISSING_EMAIL);
		} else

			try {
				if (StorageService.insertUser(pseudo, password, first_name,
						last_name, email)) {
				...
				// When special characters are used in pseudo, password,
	...
	@Produces(MediaType.APPLICATION_JSON)
	// Query parameters are parameters:
	// http://92.222.33.38:8080/app_server/ns/addtag?pseudo=abc&password=abc&object_name=xyz&picture=url
	public String addTag(@QueryParam("pseudo") String pseudo,
			@QueryParam("password") String password,
			@QueryParam("id") String id,
			@QueryParam("object_name") String object_name,
			@QueryParam("picture") String picture) throws Exception,
			JSONException {
		JSONObject obj = new JSONObject();
		...
		} else if (!FieldVerifier.verifyTagName(object_name)) {
	...
	// Query parameters are parameters:
	// http://92.222.33.38:8080/app_server/ns/addtag?pseudo=abc&password=abc&object_name=xyz&picture=url
	public String addTagWithPhoto(@FormDataParam("file") InputStream picture,
		...
		if (!FieldVerifier.verifyName(pseudo)) {
			...
			obj.put("returnCode", ErrorCode.MISSING_TAG_NAME);
		} else if (!FieldVerifier.verifyName(picture_name)) {
	...
	@Produces(MediaType.APPLICATION_JSON)
	// Query parameters are parameters:
	// http://92.222.33.38:8080/app_server/ns/removetagfromprofile?pseudo=abc&password=abc&id=xyz&profile_name=abc
	public String removeTagFromProfile(@QueryParam("pseudo") String pseudo,
			@QueryParam("password") String password,
			@QueryParam("id") String id,
			@QueryParam("profile_name") String profileName) throws Exception,
			JSONException {
		JSONObject obj = new JSONObject();
		...
		if (Utilities.isNotNull(pseudo) && Utilities.isNotNull(password)) {
			if (StorageService.checkLogin(pseudo, password)) {
				...
				int profileID = StorageService
						.getProfileID(pseudo, profileName);
	...
	@Produces(MediaType.APPLICATION_JSON)
	// Query parameters are parameters:
	// http://92.222.33.38:8080/app_server/ns/removeprofile?pseudo=abc&password=abc&profile_name=abc
	public String removeProfile(@QueryParam("pseudo") String pseudo,
			@QueryParam("password") String password,
			@QueryParam("profile_name") String profileName) throws Exception,
			JSONException {
		JSONObject obj = new JSONObject();
	...
	@Produces(MediaType.APPLICATION_JSON)
	// Query parameters are parameters:
	// http://92.222.33.38:8080/app_server/ns/modifyprofilename?pseudo=abc&password=abc&id=xyz&profile_name=abc&new_profile_name=abc
	public String modifyProfileName(@QueryParam("pseudo") String pseudo,
			@QueryParam("password") String password,
		...
		if (!FieldVerifier.verifyName(pseudo)) {
			...
			obj.put("returnCode", ErrorCode.MISSING_TAG_ID);
		} else if (!FieldVerifier.verifyName(newProfileName)) {
			obj.put("returnCode", ErrorCode.MISSING_PROFILE_NAME);
		}
	...
	@Produces(MediaType.APPLICATION_JSON)
	// Query parameters are parameters:
	// http://92.222.33.38:8080/app_server/ns/retrievetag?pseudo=abc&password=abc
	public String retrieveTags(@QueryParam("pseudo") String pseudo,
	...
	@Produces(MediaType.APPLICATION_JSON)
	// Query parameters are parameters:
	// http://92.222.33.38:8080/app_server/ns/modifyAccount?pseudo=abc&password=xyz&newPseudo=abc&newPassword=xyz&newFirstName=abc&newLastName=abc&newEmail=abc@xyz.com
	public String modifyEmail(@QueryParam("pseudo") String pseudo,
			@QueryParam("password") String password,
			@QueryParam("new_email") String newEmail) throws Exception,
			JSONException {
		JSONObject obj = new JSONObject();
		...
		if (!FieldVerifier.verifyName(pseudo)) {
			...
			obj.put("returnCode", ErrorCode.MISSING_PASSWORD);
		} else if (!FieldVerifier.verifyEMailAddress(newEmail)) {
			obj.put("returnCode", ErrorCode.MISSING_EMAIL);
		} else
	...
	@Produces(MediaType.APPLICATION_JSON)
	// Query parameters are parameters:
	// http://92.222.33.38:8080/app_server/ns/modifypassword?pseudo=abc&password=xyz&newpassword=abc
	public String modifyPassword(@QueryParam("pseudo") String pseudo,
			@QueryParam("password") String password,
			@QueryParam("new_password") String newPassword) throws Exception,
			JSONException {
		JSONObject obj = new JSONObject();
		...
		if (!FieldVerifier.verifyName(pseudo)) {
			...
			obj.put("returnCode", ErrorCode.MISSING_PASSWORD);
		} else if (!FieldVerifier.verifyName(newPassword)) {
			obj.put("returnCode", ErrorCode.MISSING_NEW_PASSWORD);
		} else if (StorageService.checkLogin(pseudo, password)) {
			if ((password != newPassword)
					&& (StorageService.modifyPassword(pseudo, newPassword))) {
	...
	// Query parameters are parameters:
	// http://92.222.33.38:8080/app_server/ns/modifypassword?pseudo=abc&password=xyz&newpassword=abc
	public String modifyObjectName(@QueryParam("pseudo") String pseudo,
			@QueryParam("password") String password,
			@QueryParam("id") String id,
		...
		if (!FieldVerifier.verifyName(pseudo)) {
			...
			obj.put("returnCode", ErrorCode.MISSING_TAG_ID);
		} else if (!FieldVerifier.verifyTagName(newObjectName)) {
	...
	@Produces(MediaType.APPLICATION_JSON)
	// Query parameters are parameters:
	// http://92.222.33.38:8080/app_server/ns/createprofile?pseudo=abc&password=xyz&profile_name=abc
	public String createProfile(@QueryParam("pseudo") String pseudo,
			@QueryParam("password") String password,
			@QueryParam("profile_name") String profileName) throws Exception,
			JSONException {
		JSONObject obj = new JSONObject();
		...
		if (!FieldVerifier.verifyName(pseudo)) {
			...
			obj.put("returnCode", ErrorCode.MISSING_PASSWORD);
		} else if (!FieldVerifier.verifyEMailAddress(profileName)) {
			obj.put("returnCode", ErrorCode.MISSING_PROFILE_NAME);
		} else
	...
	public String addTagsToProfile(@FormDataParam("pseudo") String pseudo,
			@FormDataParam("password") String password,
		...
		if (!FieldVerifier.verifyName(pseudo)) {
			...
			if (StorageService.checkLogin(pseudo, password)) {
				// on effecctue l'insertion et un bool2 à côté pour le message
				// d'erreur, qu'on fait après pour conserver la chaîne de
				// if/else
				boolean bool2 = false;
				for (int i1 = 0; i1 < listUIDs.size(); i1++) {
					if (StorageService.insertTagToProfile(pseudo, profileName,
							listUIDs.get(i))) {
						bool2 = true;
					}
				}
	...
	// Query parameters are parameters:
	// http://92.222.33.38:8080/app_server/ns/addtag?pseudo=abc&password=abc&object_name=xyz&picture=url
	public String addTagToProfile(@QueryParam("pseudo") String pseudo,
			@QueryParam("password") String password,
			@QueryParam("profile_name") String profileName,
			@QueryParam("id") String id) throws Exception, JSONException {
		JSONObject obj = new JSONObject();
	...
	@Produces(MediaType.APPLICATION_JSON)
	// Query parameters are parameters:
	// http://92.222.33.38:8080/app_server/ns/retrieveprofile?pseudo=abc&password=abc&profile_name=xyz
	public String retrieveProfile(@QueryParam("pseudo") String pseudo,
			@QueryParam("password") String password,
			@QueryParam("profile_name") String profileName) throws Exception,
			JSONException {
		JSONObject obj = new JSONObject();
